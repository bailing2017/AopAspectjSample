package com.bailing.aopaspectjsample.widget;
...
/**
 * 自定义FragmentTabHost，使用这个的话切换fragment调用hide和show方法，fragment的生命周期方法不会被调用
 * 避免重新刷新ui
 */
public class CusFragmentTabHost extends TabHost implements TabHost.OnTabChangeListener {
    ...
    static final class TabInfo {
        ...
        TabInfo(@NonNull String _tag, @NonNull Class<?> _class, @NonNull Bundle _args) {
            tag = _tag;
            clss = _class;
            args = _args;
        }
    ...
    private void initFragmentTabHost(Context context, AttributeSet attrs) {
        TypedArray
            a = context.obtainStyledAttributes(attrs, new int[]{android.R.attr.inflatedId}, 0, 0);
    ...
    private void ensureContent() {
        if (mRealTabContent == null) {
            ...
            if (mRealTabContent == null) {
                throw new IllegalStateException("" + mContainerId);
            }
        }
    }
    ...
    public void addTab(@NonNull TabHost.TabSpec tabSpec, @NonNull Class<?> clss, @NonNull
        Bundle args) {
    ...
    protected void onAttachedToWindow() {
        ...
        for (int i = 0; i < mTabs.size(); i++) {
            ...
            if (tab.fragment != null) {
                if (tab.tag != null && tab.tag.equals(currentTab)) {
    ...
    private FragmentTransaction doTabChanged(String tabId, FragmentTransaction ft) {
        ...
        for (int i = 0; i < mTabs.size(); i++) {
            TabInfo tab = mTabs.get(i);
            if (tab != null && tab.tag != null && tab.tag.equals(tabId)) {
                newTab = tab;
            }
        }
        if (newTab == null) {
